...
- **API response trimming**: `web.ts` strips heavy fields (`description`, `commentsJson`, `dataJson`, `diffSummary`, nested commit/ticket arrays) at the serialization boundary before sending JSON. Query functions return full data for CLI callers. Reduces initial dashboard load from ~4.5MB to <500KB
- **DB column selection (lite mode)**: Query functions accept optional `{ lite: true }` to exclude heavy text columns (`commentsJson`, `dataJson`, `diffSummary`) at the SQL level. `{ noRelations: true }` skips commit/ticket batch-fetches entirely (used where only branch rows are needed). `web.ts` passes these opts at all dashboard call sites; CLI callers omit them to get full data. Eliminates unnecessary Supabase egress for dashboard queries
